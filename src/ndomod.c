                           ...
                           )
{
    ...
    if (mg_ret == 0) {
        ...
        memcpy(*instance_id_ret, instance_id, strlen(instance_id));
        
        if(strlen(instance_id) > 0) {
            bson query_objects[1];
            bson_init(query_objects);
            //bson_append_string(query_objects, "name1", (es[0]==NULL)?"":es[0]);
            bson_append_string(query_objects, "name1", host_name);
            if(service_name != NULL) {
                bson_append_string(query_objects, "name2", service_name);
            }
            ...
            if(mg_ret == 0) {
                ...
                memcpy(*object_id_ret, obj_id, strlen(obj_id));
            }
            // if cannot found
            // insert record to table 'objects'
                ...
                // add hostname
                bson_append_string(b, "name1", host_name);
                // add service_name
                if(service_name != NULL) {
...
int ndomod_broker_data(int event_type, void *data){
    ...
    
    {
        ...
        /*
        char *string_buf = calloc(sizeof(char), 512);
        sprintf(string_buf, "In service_check: instance_id: %s, object_id: %s",
	...
	/*
        ...
        */
        ...
        time_t timet;
        time(&timet);
	bson_append_time_t(b, "last_update", timet + 28800);
	...
	if(es[7] != NULL) {
	    ...
	    bson_append_time_t(perf, "last_update", timet + 28800);
        ...
        /* start create json object*/
    {
        ...
        cJSON_AddNumberToObject(json_data, "attr", hcdata->attr);
        
        cJSON_AddStringToObject(json_data, "host", (es[0]==NULL)?"":es[0]);
        
        cJSON_AddNumberToObject(json_data, "check_type", hcdata->check_type);
        cJSON_AddNumberToObject(json_data, "current_attempt", hcdata->current_attempt);
        cJSON_AddNumberToObject(json_data, "max_attempts", hcdata->max_attempts);
        cJSON_AddNumberToObject(json_data, "state_type", hcdata->state_type);
        cJSON_AddNumberToObject(json_data, "state", hcdata->state);
        cJSON_AddNumberToObject(json_data, "timeout", hcdata->timeout);
        
        cJSON_AddStringToObject(json_data, "command_name", (es[1]==NULL)?"":es[1]);
        cJSON_AddStringToObject(json_data, "command_args", (es[2]==NULL)?"":es[2]);
        cJSON_AddStringToObject(json_data, "command_line", (es[3]==NULL)?"":es[3]);
        
        cJSON_AddNumberToObject(json_data, "return_code", hcdata->return_code);
        
        cJSON_AddStringToObject(json_data, "output", (es[4]==NULL)?"":es[4]);
        cJSON_AddStringToObject(json_data, "long_output", (es[5]==NULL)?"":es[5]);
        cJSON_AddStringToObject(json_data, "perfdata", (es[6]==NULL)?"":es[6]);
        
        // send JSON message to RabbitMQ server
        json_out = cJSON_PrintUnformatted(json_root);
        
        /*
        char *string_buf = calloc(sizeof(char), 4096);
        memset(string_buf, 0, 4096);
        sprintf(string_buf, "host_check is here: %s", json_out);
        ndomod_write_to_logs(string_buf, NSLOG_INFO_MESSAGE);
        free(string_buf);
        */
    ...
    // insert performance data to nagios_host_perfdata
    if(es[6] != NULL) {
        char *instance_id_global = (char *)calloc(sizeof(char), 512);
        char *object_id_global = (char *)calloc(sizeof(char), 512);
        get_instance_object_id((ndomod_instance_name==NULL)?"default":ndomod_instance_name,
	...
	if(mongo_insert(mongo_conn, "wisemonitor.nagios_host_perfdata", perf, NULL) != MONGO_OK) {
	    char *string_buf = calloc(sizeof(char), 512);
	    sprintf(string_buf, "MongoDB insert data error: %s", mongo_conn->lasterrstr);
	    ndomod_write_to_logs(string_buf, NSLOG_SERVICE_WARNING);
	    free(string_buf);
    ...
    
    {
        ...
        /*
        char *string_buf = calloc(sizeof(char), 512);
        sprintf(string_buf, "In host_check: instance_id: %s, object_id: %s",
                instance_id_global, object_id_global);
        ndomod_write_to_logs(string_buf, NSLOG_SERVICE_WARNING);
        free(string_buf);
        */
        ...
        bson_finish(query);
        mg_ret = mongo_find_one(mongo_conn,
                                "wisemonitor.nagios_host_status",
                                query,
                                bson_shared_empty(),
                                NULL
                               );
        
        // Insert host check information to mongodb
        bson b[1];
        bson_init(b);
        if(mg_ret != 0) {
            // if is a new record
	...
	bson_append_time_t(b, "last_update", timet + 28800);
        ...
        bson_append_int(b, "check_type", hcdata->check_type);
        bson_append_int(b, "current_attempt", hcdata->current_attempt);
        bson_append_int(b, "max_attempts", hcdata->max_attempts);
        bson_append_int(b, "state_type", hcdata->state_type);
        bson_append_int(b, "state", hcdata->state);
        bson_append_int(b, "timeout", hcdata->timeout);
        
        bson_append_string(b, "command_name", (es[1]==NULL)?"":es[1]);
        bson_append_string(b, "command_args", (es[2]==NULL)?"":es[2]);
        bson_append_string(b, "command_line", (es[3]==NULL)?"":es[3]);
        
        bson_append_int(b, "return_code", hcdata->return_code);
        
        bson_append_string(b, "output", (es[4]==NULL)?"":es[4]);
        bson_append_string(b, "long_output", (es[5]==NULL)?"":es[5]);
        bson_append_string(b, "perfdata", (es[6]==NULL)?"":es[6]);
        bson_finish(b);
        
        if(mg_ret != 0) {
            if(mongo_insert(mongo_conn, "wisemonitor.nagios_host_status", b, NULL) != MONGO_OK) {
                char *string_buf = calloc(sizeof(char), 512);
                sprintf(string_buf, "MongoDB insert data error: %s", mongo_conn->lasterrstr);
                ndomod_write_to_logs(string_buf, NSLOG_SERVICE_WARNING);
                free(string_buf);
...
int ndomod_write_object_config(int config_type){
	...
	for(temp_host=host_list;temp_host!=NULL;temp_host=temp_host->next){
        ...
        cJSON_AddStringToObject(data, "host_alias", (es[1]==NULL)?"":es[1]);
        cJSON_AddStringToObject(data, "host_address", (es[2]==NULL)?"":es[2]);
        cJSON_AddStringToObject(data, "host_check_command", (es[3]==NULL)?"":es[3]);
        cJSON_AddStringToObject(data, "host_event_handler", (es[4]==NULL)?"":es[4]);
        cJSON_AddStringToObject(data, "host_notification_period", (es[5]==NULL)?"":es[5]);
        cJSON_AddStringToObject(data, "host_check_period", (es[6]==NULL)?"":es[6]);
        cJSON_AddStringToObject(data, "host_failure_prediction_options", (es[7]==NULL)?"":es[7]);
        
        cJSON_AddNumberToObject(data, "host_check_interval", (double)temp_host->check_interval);
        cJSON_AddNumberToObject(data, "host_retry_interval", (double)retry_interval);
        ...
        */
        time_t timep;
        time(&timep);
        bson_append_time_t(b, "last_update", timep + 28800);
        bson_append_long(b, "timestamp", timestamp);
